	...
	public StudentVO getStudentSubjectName(String sd_id)throws Exception{
		...
		try {
			...
			if(rs.next()) {
				...
				studentInfo.setSd_num("sd_num");
				studentInfo.setSd_name("sd_name");
				studentInfo.setS_num("s_num");
			}
...
//선택한 과목명의 과목 번호
	...
	public void getTraineeRegiste(TraineeVO tvo)throws Exception{
		String sql="insert into trainee"+"(no,sd_num,l_num,t_section,t_date)"+"values"+"(trainee_seq.nextval,?,?,?,sysdate)";
		Connection con=null;
		PreparedStatement pstmt=null;
	...
	public ArrayList<TraineeVO>getTraineeStudentNumSearchList(String sd_num)throws Exception{
		...
		try {
			...
			while(rs.next()) {
				...
				tVo.setNo(rs.getInt("no"));
				tVo.setSd_num(rs.getString("sd_num"));
				tVo.setSd_name(rs.getString("l_num"));
				tVo.setL_num(rs.getString("t_section"));
				tVo.setT_date(rs.getString("t_date"));
